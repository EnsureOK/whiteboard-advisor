...
> 🔒 **安全评估(2026-09-06)**:launcher.py 读自身环境变量/写数据目录/健康检查本机后端,
> 经 Mimosa 深度扫描分诊为本地单机应用下可接受(无可信输入源)。桌面包为单用户形态,
> 数据不出本机;如需多人共用请部署 Web 版并设 `AUTH_REQUIRED=true`。详见 docs/SPEC.md §9。
